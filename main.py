...
class HyperliquidClient:
    ...
    def get_spot_pairs(self):
        ...
        if not meta: return []
        # Build token name map from 'tokens' array (index → name)
        ...
        for i, t in enumerate(meta.get('universe', [])):
            # universe entry has 'tokens' list of 2 token indices [base, quote]
            tok_indices = t.get('tokens', [])
            name = t.get('name', '')
            if not name and tok_indices:
                # base token index → name (skip USDC which is quote)
                base_idx = tok_indices[0]
                name = token_names.get(base_idx, '')
            if not name:
                continue   # skip unnamed pairs
            pairs.append({'index': i, 'name': name, 'asset_id': 10000+i})
