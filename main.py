...
class HyperliquidClient:
    ...
    def get_spot_meta(self, force=False):
        ...
        if data:
            ...
            self._meta_ts    = now
            # Build token name → universe index map
            # tokens array: [{name: "SOL", index: 5, ...}, ...]
            # universe array: [{name: "SOL/USDC", tokens: [5, 0], ...}, ...]
            tokens = data.get('tokens', [])
            # token_idx_to_name: token index → token name (e.g. 5 → "SOL")
            token_idx_to_name = {t.get('index'): t.get('name','').upper()
                                  for t in tokens if t.get('name')}
            idx_map = {}
            for i, p in enumerate(data.get('universe', [])):
                uni_name = p.get('name', '').upper()  # e.g. "SOL/USDC"
                tok_idxs = p.get('tokens', [])
                # Add universe name variants
                if uni_name:
                    idx_map[uni_name] = i
                    # Also add just base part: "SOL/USDC" → "SOL"
                    base = uni_name.split('/')[0].strip()
                    if base:
                        idx_map[base] = i
                # Add base token name from tokens array
                if tok_idxs:
                    base_tok_name = token_idx_to_name.get(tok_idxs[0], '')
                    if base_tok_name:
                        idx_map[base_tok_name] = i
    ...
    def get_spot_pairs(self):
        ...
        # token index → clean name map
        token_idx_to_name = {t.get('index'): t.get('name','')
                              for t in tokens if t.get('name')}
        pairs = []
        seen = set()
        for i, t in enumerate(meta.get('universe', [])):
            tok_indices = t.get('tokens', [])
            uni_name = t.get('name', '')
            # Get base token name (e.g. SOL from SOL/USDC)
            if uni_name and '/' in uni_name:
                clean = uni_name.split('/')[0].strip()
            elif uni_name:
                clean = uni_name
            elif tok_indices:
                clean = token_idx_to_name.get(tok_indices[0], '')
            else:
                clean = ''
            # Skip empty, USDC itself, duplicates, @index style
            if not clean or clean == 'USDC' or clean in seen or clean.startswith('@'):
                continue
            seen.add(clean)
            pairs.append({'index': i, 'name': clean, 'asset_id': 10000+i})
...
class BotEngine:
    ...
    def start(self, wallet=None, private_key=None):
        ...
        # Start async engine (WS + cache + decision loop)
        self._async_eng.start()
        # Warm up candle cache immediately
        coins = list(self.coins.keys())
        if coins:
            self._async_eng.trigger_cache_refresh(coins)
        self._push_event('monitor', f"Bot started in {mode} mode (PARALLEL ENGINE)", {'mode':mode})
        return {'success':True,'message':f'Bot started ({mode} + parallel engine)','mode':mode}
