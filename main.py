...
class HyperliquidClient:
    ...
    def get_spot_price(self, symbol):
        ...
        allMids uses @{spot_universe_index} format for spot (NOT 10000+i).
        """
        mids = self.get_all_mids()
        if not mids:
            return None
        # Correct spot format: @{index} where index = position in spotMeta universe
        try:
            meta = self.get_spot_meta()
            if meta:
                for i, pair in enumerate(meta.get('universe', [])):
                    if pair.get('name', '').upper() == symbol.upper():
                        spot_key = f"@{i}"
                        if spot_key in mids:
    ...
    def get_candles(self, symbol, interval, lookback=100):
        ...
        candleSnapshot uses @{universe_index} format (e.g. @4 for SOL/USDC).
        NOT @{10000+index} — that format is only for exchange orders.
        ...
        # Resolve @{universe_index} for candleSnapshot (NOT 10000+i)
        coin_id = symbol
        try:
            meta = self.get_spot_meta()
            if meta:
        ...
        raw = self._post({
            ...
            "req": {"coin": coin_id, "interval": tf,
                    "startTime": int(start_ms), "endTime": int(now_ms)}
        })
        if not raw:
            return []
        ...
        for c in raw:
            if isinstance(c, dict):
                # Official keys: t=open_ms, T=close_ms, o,h,l,c,v (all lowercase except T)
                candles.append([
                    c.get('t', 0),
                    float(c.get('o', 0)),
                    float(c.get('h', 0)),
