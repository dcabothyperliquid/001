...
class HyperliquidClient:
    ...
    def get_spot_price(self, symbol):
        """Get current price for a spot symbol using @index format"""
        mids = self.get_all_mids()
        if not mids:
            return None

        # First try direct symbol keys (sometimes works for popular coins)
        ...
        # Correct method: find @index from spotMeta and look up in allMids
        try:
            meta = self.get_spot_meta()
            if meta:
                for i, token in enumerate(meta.get('universe', [])):
            ...
            logger.error(f"Spot price lookup error {symbol}: {e}")
    ...
    def get_candles(self, symbol, interval, lookback=100):
        ...
        Hyperliquid spot candles require @index format (e.g. @10164 for SOL).
        API response keys: t(open_time), T(close_time), o, h, l, c, v — mixed case.
        ...
        try:
            ...
            logger.warning(f"Could not resolve spot index for {symbol}, using raw symbol: {e}")
        ...
        # Hyperliquid candle response uses mixed case keys:
        # t = open time (ms), T = close time (ms), o = open, h = high, l = low, c = close, v = volume
        candles = []
        for c in raw:
            if isinstance(c, dict):
                candles.append([
                    c.get('t', 0),   # 0: open timestamp
                    c.get('o', 0),   # 1: open
                    c.get('h', 0),   # 2: high
                    c.get('l', 0),   # 3: low
                    c.get('c', 0),   # 4: close ✅
