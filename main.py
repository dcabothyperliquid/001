...
class HyperliquidClient:
    ...
    def get_spot_meta(self, force=False):
        ...
        if data:
            ...
            self._meta_ts    = now
            # Real Hyperliquid structure:
            # tokens: [{name:"PURR",index:0},{name:"SOL",index:1},{name:"USDC",index:3}...]
            # universe: [{tokens:[0,3],...},{tokens:[1,3],...}]
            # universe[i].tokens[0] = base token index in tokens array
            tokens_list = data.get('tokens', [])
            # Map: token_index → token_name
            token_idx_to_name = {}
            ...
            logger.info(f"spotMeta loaded: {len(idx_map)} symbols, e.g. {list(idx_map.keys())[:5]}")
        return data
    ...
    def get_spot_pairs(self):
        ...
        for i, u in enumerate(meta.get('universe', [])):
            ...
            # Base token name from tokens array
            clean = ''
            if tok_indices:
            ...
            if not clean:
                uni_name = u.get('name', '')
                clean = uni_name.split('/')[0].strip() if '/' in uni_name else uni_name.strip()
            # Skip USDC, empty, duplicates
            if not clean or clean.upper() in ('USDC', '') or clean in seen:
                continue
            seen.add(clean)
